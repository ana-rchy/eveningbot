...
pub async fn init_jobs(sched: Arc<JobScheduler>, client: &serenity::Client) -> Result<(), JobSchedulerError> {
        ...
        let job = JobBuilder::new()
            ...
            .with_run_async(Box::new(move | _uuid, _l | {
                ...
                Box::pin(async move {
                    ...
                    let job = create_sunset_job(http, GENERAL_CHANNEL_ID, evening_bag.clone()).await;
...
async fn create_sunset_job(http: Arc<Http>, channel_id: u64, bag: Arc<Mutex<Vec<&'static str>>>) -> tokio_cron_scheduler::Job {
    let time = web::get_sunset_time().await.unwrap();
    let schedule = &format!("{} {} {} * * *", time.second(), time.minute(), time.hour())[..];
    println!("sunset today - {:02}:{:02}:{:02}", time.hour(), time.minute(), time.second());
